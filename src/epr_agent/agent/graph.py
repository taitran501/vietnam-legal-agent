...
from epr_agent.tools.evidence import EvidenceEvaluator, explicit_article_ids, verify_citations
...
def build_workflow(deps: WorkflowDependencies):
    ...
    async def ask_user(state: AgentState) -> AgentState:
        ...
        _trace(state, reason_code="required_case_facts_missing", payload={"missing_facts": list(state.get("missing_facts") or [])})
        return state
    ...
    async def retrieve_legal(state: AgentState) -> AgentState:
        ...
        try:
            ...
            # The retriever returns its top 10 ranked candidates.  Keep only
            # the first 3 evidence chunks in the generation surface while
            # retaining all candidate scores in the trace.
    ...
    async def evaluate_evidence(state: AgentState) -> AgentState:
        ...
        assessment = deps.evidence.evaluate(
            ...
            docs,
            state["task_type"],
            expected_articles=set(state.get("explicit_articles") or []),
        )
    ...
    async def retrieve_web(state: AgentState) -> AgentState:
        ...
        try:
            ...
            state["evidence_assessment"] = deps.evidence.evaluate(state["standalone_query"], docs, state["task_type"]).to_dict()
...
async def create_initial_state(
    ...
    *,
    ...
    return {
        ...
        "pipeline_version": "legal-first-v2",
