# Autonomous Agent Architecture & Evaluation Harness

This document details the **True Autonomous Agent** architecture (ReAct Cognitive Loop) and the **Trajectory Evaluation Harness** integrated into EPR Copilot under the `pipeline-agent` pipeline version.
...
| **Tool Calling** | Hardcoded logic invoking retrieval or rule packs per route. | **Dynamic Tool Registry**: 7 standardized tools selected dynamically based on information needs. |
...
flowchart TD
    ...
    RouteCheck -->|Complex EPR Task| AgentRunner[EprAgentRunner ReAct Loop]
...
The autonomous agent is equipped with 7 standardized tools:
...
4. `evaluate_epr_obligation`: Evaluates legal obligations and exemptions against the EPR Rule Pack.
5. `get_case_form_fields`: Retrieves case form requirements and identifies missing facts for a scenario.
6. `load_conversation_context`: Loads conversation history and active case facts.
7. `ask_user_for_clarification`: Requests missing facts from the user (terminal action).
...
- **Misconception Resolution**: Proactively explains that end-use retailers (e.g. bubble tea shops or restaurants using packaging) are packaging consumers, **not packaging producers**, and are therefore not subject to EPR fees.
