...
domains, including EPR; this documentation does not constitute official legal
...
- **Domain Contract:** Source code in `src/epr_agent/domain/`, particularly `v4.py`, `legal_rules.py`, and the EPR-specific `epr_rules.py`, is the source of truth for fields, dependencies, validation, and case states. The route `backend/api/routes/case_form.py` is a side-effect-free HTTP adapter.
